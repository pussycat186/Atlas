...
// Backend URL - will be updated when deployment is available
const BACKEND_BASE = __ENV.BACKEND_URL || 'http://localhost:3000';

export default function() {
  // Test the health endpoint
  const response = http.get(`${BACKEND_BASE}/health`);
  
  const success = check(response, {
    'status is 200': (r) => r.status === 200,
    'response time < 200ms': (r) => r.timings.duration < 200,
    'has valid response': (r) => {
      return r.body && r.body.includes('healthy');
    },
  });
...
export function handleSummary(data) {
  return {
    '_reports/k6-backend-summary.json': JSON.stringify({
