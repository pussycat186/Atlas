...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Service Health Constellations</CardTitle>
          <CardDescription>
            ...
            {clusterHealth.services.map((service, index) => (
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <Badge 
                    variant={service.status === 'healthy' ? 'secondary' : service.status === 'degraded' ? 'secondary' : 'destructive'} 
                    role="status" 
              ...
              {witnessQuorum.witnesses.map((witness) => (
                <div key={witness.id} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <Badge variant={witness.status === 'active' ? 'secondary' : witness.status === 'lagging' ? 'secondary' : 'destructive'} role="status" aria-label={`Witness status: ${witness.status}`}>
                      {witness.status}
                    </Badge>
                  </div>
                </div>
